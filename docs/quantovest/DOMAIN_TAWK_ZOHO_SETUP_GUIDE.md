...
The Quantovest application uses Next.js App Router and now includes `components/TawkToWidget.tsx`. The integration reads `NEXT_PUBLIC_TAWK_PROPERTY_ID` and `NEXT_PUBLIC_TAWK_WIDGET_ID` from the environment, loads the widget client-side, prevents duplicate script injection during navigation, hides the launcher on `/admin` routes, and applies mobile-safe sizing. The public identifiers belong in Vercel environment variables, not hardcoded source files.

For reference, the client-safe pattern is:
...
The component is mounted from the root layout and is intentionally suppressed on admin routes. After adding the final domain, configure Tawk’s domain restriction to the production host and test both the root and `www` host if both are served. If the app has a strict Content Security Policy, allow the Tawk.to script and related resources explicitly; otherwise the browser console may report CSP blocks. [4]
