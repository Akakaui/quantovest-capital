    ...
    <div className="min-h-screen bg-[#0D1215] text-[#E8EFEB] flex flex-col md:flex-row font-sans">
      ...
      <main className="flex-1 p-4 sm:p-8 space-y-8 overflow-y-auto pb-24 md:pb-8">
        <div className="border-b border-[#2B393F] pb-6">
          ...
          <p className="mt-1 text-xs text-[#93A09A]">Review submitted identity documents and audit approved or declined applications.</p>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h2 className="text-base font-semibold">Identity Applications</h2>
        ...
        {loading ? (
          ...
          <EmptyState
            title={view === 'pending' ? 'No pending KYC applications' : 'No KYC history'}
            hint="Submitted identity documents will appear here for verification."
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {row.documentUrls?.map((url, index) => <a key={url} href={url} target="_blank" rel="noreferrer" className="rounded-xl border border-[#2B393F] px-3 py-2 text-xs text-[#A8B6AD] hover:bg-[#0D1215]">View {index === 0 ? 'ID document' : 'proof of address'}</a>)}
                      {!row.documentUrls?.length && <span className="text-xs text-amber-300">Documents unavailable for preview</span>}
                    </div>
                    {row.reviewNote && <p className="mt-3 text-xs text-[#A8B6AD]">Review note: {row.reviewNote}</p>}
                  </div>
                  {row.status === 'pending' && (
                    <div className="flex gap-2">
                      <button type="button" onClick={() => void review(row.id, 'approve')} className="rounded-full bg-[#22C55E] px-4 py-2 text-xs font-semibold text-[#07110B] hover:bg-[#86EFAC]">Approve</button>
                      <button type="button" onClick={() => void review(row.id, 'decline')} className="rounded-full border border-rose-400/40 px-4 py-2 text-xs text-rose-300 hover:bg-rose-500/10">Decline</button>
                    </div>
                  )}
                </div>
              </article>
