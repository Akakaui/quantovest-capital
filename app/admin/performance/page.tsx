...
function isSameUtcDay(value: string) {
...
export default function AdminPerformancePage() {
  ...
  const alreadyCreditedToday = Boolean(lastCredit && isSameUtcDay(lastCredit.entryDate));
  ...
  async function publishFixed() {
    if (!selected || !planRoi || alreadyCreditedToday || !marketNote.trim()) return;
    setSubmitting(true);
    setMessage('');
    try {
      const response = await fetch('/api/admin/roi', {
        ...
        body: JSON.stringify({ investorId: selected.id, percentageBps: planRoi.daily * 100, marketNote: marketNote.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0D1215] text-[#E8EFEB] flex flex-col md:flex-row font-sans">
      ...
      <main className="flex-1 p-4 sm:p-8 space-y-8 overflow-y-auto pb-24 md:pb-8">
        <div className="border-b border-[#2B393F] pb-6 space-y-1">
          ...
          <p className="text-xs text-[#93A09A]">Select an investor, review the last credit, and apply no more than one plan performance credit per UTC day.</p>
        </div>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-[#2B393F] bg-[#151E23] p-5 sm:p-6">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-semibold ${alreadyCreditedToday ? 'bg-amber-500/10 text-amber-300' : 'bg-[#22C55E]/10 text-[#86EFAC]'}`}>
                  <span className={`h-2 w-2 rounded-full ${alreadyCreditedToday ? 'bg-amber-400' : 'bg-[#22C55E]'}`} />
                  {alreadyCreditedToday ? 'Already credited today' : 'Ready for today'}
                </div>
              </div>
              <div className="mt-5 grid grid-cols-2 lg:grid-cols-5 gap-3">
                {[['Plan', selected.planName ?? 'None'], ['Balance', formatCents(selected.balanceCents)], ['Rate', planRoi ? `${planRoi.daily}%` : 'N/A'], ['Last credit', lastCredit ? formatDate(lastCredit.entryDate) : 'None'], ['Last amount', lastCredit ? formatCents(lastCredit.profitCents) : '—']].map(([label, value]) => <div key={label} className="rounded-xl bg-[#0D1215] border border-[#2B393F] p-3"><p className="text-[10px] uppercase font-mono text-[#7F8C86]">{label}</p><p className="mt-1 text-xs font-mono font-semibold text-white break-words">{value}</p></div>)}
              </div>
            </div>

            {planRoi ? (
              <div className="rounded-2xl border border-[#22C55E]/30 bg-[#22C55E]/5 p-6 sm:p-8 space-y-5">
                <div className="flex items-center gap-3"><div className="w-12 h-12 rounded-full bg-[#22C55E]/10 flex items-center justify-center"><Icon icon="solar:chart-up-bold" className="w-7 h-7 text-[#22C55E]" /></div><div><h3 className="text-xl font-semibold text-white">Today&apos;s performance credit</h3><p className="text-xs text-[#93A09A]">Fixed {planRoi.daily}% rate for the {selected.planName} plan</p></div></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs"><div className="rounded-xl bg-[#0D1215]/70 border border-[#2B393F] p-4"><p className="text-[#93A09A]">Estimated credit</p><p className="mt-1 text-xl font-mono font-bold text-[#22C55E]">{formatCents(Math.round(dailyProfit * 100))}</p></div><div className="rounded-xl bg-[#0D1215]/70 border border-[#2B393F] p-4"><p className="text-[#93A09A]">After credit</p><p className="mt-1 text-xl font-mono font-bold text-white">{formatCents(Math.round((balance + dailyProfit) * 100))}</p></div></div>
                ...
                <button onClick={() => void publishFixed()} disabled={submitting || alreadyCreditedToday || !marketNote.trim()} className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-[#22C55E] text-[#0A0D0C] font-semibold text-sm hover:bg-[#16A34A] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">{submitting ? 'Applying…' : alreadyCreditedToday ? 'Already credited today' : 'Apply today’s performance credit'}</button>
              </div>
        ...
        <section className="rounded-2xl border border-[#2B393F] bg-[#151E23] p-5 space-y-4"><h2 className="text-sm font-semibold text-white">Fixed performance reference</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-3">{Object.entries(PLAN_ROI).map(([plan, { daily, label }]) => <div key={plan} className="rounded-xl bg-[#0D1215] border border-[#2B393F] p-4 text-center"><p className="text-xs text-[#93A09A]">{plan}</p><p className="text-2xl font-mono font-bold text-[#22C55E] mt-1">{daily}%</p><p className="text-[10px] text-[#7F8C86] mt-1">{label}</p></div>)}</div></section>
      </main>
    </div>
  );
}
