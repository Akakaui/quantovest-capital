...
export default function AdminReferralPayoutsPage() {
  ...
  async function load() { setLoading(true); const response = await fetch('/api/admin/referrals/withdrawals', { cache: 'no-store' }); if (response.ok) setRows(await response.json()); else setMessage('Unable to load the referral payout records.'); setLoading(false); }
  useEffect(() => { void load(); }, []);
