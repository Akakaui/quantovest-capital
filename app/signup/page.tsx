...
export default function SignupPage() {
  ...
  async function handleSignup(event: React.FormEvent) {
    ...
    const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { name }, emailRedirectTo: `${window.location.origin}/auth/callback?next=/dashboard` } });
    if (error) setMessage(error.message); else if (data.session) router.push('/dashboard'); else setMessage('Check your email to confirm your account, then sign in.');
    setLoading(false);
  }
