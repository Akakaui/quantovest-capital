...
export async function PATCH(request: Request) {
  ...
  try {
    await db.transaction(async tx => {
      ...
      await tx.update(referralWithdrawals).set({ status, reviewedBy: session.user.id!, reviewNote: body.reviewNote?.trim() || null }).where(eq(referralWithdrawals.id, withdrawal.id));
