...
const MIN_WITHDRAWAL_CENTS = 150_000;
...
export async function POST(request: Request) {
  try {
    ...
    if (!body || typeof body.amountCents !== "number" || !Number.isInteger(body.amountCents) || body.amountCents < MIN_WITHDRAWAL_CENTS) return NextResponse.json({ error: "Minimum referral withdrawal is $1,500." }, { status: 400 });
    ...
    try {
      const result = await db.transaction(async tx => {
        ...
        if (available < MIN_WITHDRAWAL_CENTS) throw new Error("Referral balance has not reached the $1,500 minimum.");
